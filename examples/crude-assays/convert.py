...
def generate():
    ...
    report = ["# Five crude assays on the existing 20-component basis", "",
              ...
              "## Mole composition", "", "Values are mol%, not mass%. Full-precision fractions are in [converted-compositions.json](examples/crude-assays/converted-compositions.json).", ""]
    ...
    report.extend(["", "Source transcriptions and PDF SHA-256 hashes: [source-data.json](examples/crude-assays/source-data.json).",
                   "Reproduce with `python examples/crude-assays/convert.py`. Validate the converter with `python examples/crude-assays/test_convert.py`.",
    ...
    (ROOT / "CRUDE_ASSAY_CONVERSION.md").write_text("\n".join(report), encoding="utf-8")
    for row in rows:
