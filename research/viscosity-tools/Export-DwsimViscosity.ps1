...
$exe = Join-Path $repo 'research/viscosity-tools/raw/Export-DwsimViscosity.exe'
...
if ($Ambient) { $exportArgs += (Join-Path $repo 'research/viscosity-tools/raw/dwsim-ambient-viscosity-export.json'); $exportArgs += '--ambient' }
else { $exportArgs += (Join-Path $repo 'research/viscosity-tools/raw/dwsim-viscosity-export.json') }
& $exe @exportArgs
