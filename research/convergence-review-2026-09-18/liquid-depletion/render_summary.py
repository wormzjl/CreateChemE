"""Renders summary.md. The narrative is held here so the deliverable is a generated artifact;
every number in it comes from summary.json / tables.md, which analyze.py writes.

Run:  python render_summary.py
"""
import os
...
def main():
    ...
    with open(path, "w", encoding="utf-8") as handle:
        handle.write(SUMMARY.lstrip("\n"))
        handle.write(SECTION8)
    print(f"wrote {path} ({len(SUMMARY.splitlines()) + len(SECTION8.splitlines())} lines)")
