#!/usr/bin/env bash
# WP7d: compiles the WP7d main sources and the named test sources with javac against the last Gradle compile of the
# worktree (build/classes, build/resources) and runs test classes with the reflective runner, without Gradle.
# Usage: bash tools/wp7d-engine-defects/run.sh <fully.qualified.TestClass> ... [-Dmethod=name]; MAIN=<class> runs a probe.
set -euo pipefail
...
"$JDK/javac" -nowarn -encoding UTF-8 -d "$(cygpath -w "$OUT")" -cp "$CP" \
  $M/thermo/phase/*.java $M/fluid/thermo/FluidThermodynamics.java \
  $T/thermo/phase/*.java $T/thermo/qualification/*.java $T/fluid/thermo/NetworkPhaseEngineTest.java \
  $T/fluid/network/NearCriticalNitrogenIslandTest.java tools/g3-qualification/RunTests.java ${EXTRA:-}
PROPS=(); CLASSES=()
