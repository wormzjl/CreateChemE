...
/** Offline native-anchor/residual transformer candidate. Parser-owned weights are immutable after validation.
 * Every prediction owns all scratch arrays; cooperative cancellation is checked inside matrix work.
 */
final class V3HybridResidualInitializer implements V3NeuralInitializer {
    private static final String REVISION = "v3-hybrid-residual-1";
    ...
    private V3HybridResidualInitializer(Document model) { this.model = model; }

    static V3HybridResidualInitializer read(InputStream stream) throws IOException {
        ...
        if (m == null || !REVISION.equals(m.featureRevision) || !"hybrid-residual".equals(m.modelType)
        ...
        for (String name : List.of("xm", "xscale", "ym", "yscale", "gm", "gscale", "bm", "bscale", "dm", "dscale")) {
        ...
        var result = new V3HybridResidualInitializer(m);
        result.checkLinear("embed", 185, 64); result.checkNorm("output.0"); result.checkLinear("output.1", 64, 85);
        result.checkLinear("branch.0", 74, 64); result.checkLinear("branch.2", 64, 3);
        for (int i = 0; i < 2; i++) {
            String p = "blocks." + i + ".";
            if (m.modelType.equals("hybrid-residual")) {
        ...
        long expected = m.modelType.equals("hybrid-residual") ? 89496 : 83288;
    ...
    Raw raw(V3ColumnInput input, V3SolveControl control) {
        ...
        for (int block = 0; block < 2; block++) {
            ...
            if (model.modelType.equals("hybrid-residual")) {
        ...
        for (int n = 0; n < hidden.length; n++) {
            ...
            for (int i = 0; i < 85; i++) values[n][i] = (i < 43 ? anchor.values()[n][i] : 0) + values[n][i] * model.normalization.get("dscale")[i] + model.normalization.get("dm")[i];
        }
    ...
    private static final class Document {
        String featureRevision, modelType, modelId, packageId, propertyRevision;
