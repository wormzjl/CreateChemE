#!/usr/bin/env bash
# F3 gates, one Gradle invocation at a time, logs in f3-logs/gate-<tag>-<name>.log. Usage: gates.sh <tag> [names...]
...
for n in $NAMES; do
  case $n in
    ...
    gametest) G gametest runFluidGameTestServer -PfluidGameTestRunId=f3-$TAG ;;
  esac
done
