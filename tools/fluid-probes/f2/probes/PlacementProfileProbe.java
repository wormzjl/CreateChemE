...
class PlacementProfileProbe {
    ...
    final PhysicalRegistry.Host host=new PhysicalRegistry.Host() {
        ...
        public void topology(String dimension,Set<UUID> events,Set<Long> affected,List<IslandCoordinator.Replacement> replacements,long committed,long online,Map<Long,PassiveNetwork.Reservoir> additions,Set<Long> removals,Map<Long,InlineFilter> releasedFilters,Runnable commit) {
            coordinator.topology(events,affected,replacements,model,committed,online,additions,removals,releasedFilters,commit);
        }
    ...
    Op measure(String name,int repeats,Runnable setup,Runnable op,Runnable teardown) {
        double[] s=new double[repeats],a=new double[repeats];long compiled=0;
        for(int i=0;i<repeats;i++){setup.run();submitNanos=0;applyNanos=0;long before=FluidRuntimeDiagnostics.compiledDevices.sum();op.run();compiled+=FluidRuntimeDiagnostics.compiledDevices.sum()-before;s[i]=submitNanos/1e6;a[i]=applyNanos/1e6;teardown.run();}
        lastCompiled=(double)compiled/repeats;return new Op(name,s,a);
    }
    double lastCompiled;
    void report(int n,Op op) {
        ...
        say(String.format(Locale.ROOT,"N=%5d %-26s median %8.3f ms (submit %7.3f, apply %7.3f)  p90 %8.3f ms  runs %d  compiled devices per event %.1f",n,op.name,median(total),median(op.submitMs),median(op.applyMs),p90(total),total.length,lastCompiled));
    }
    @Test void placementCostAgainstWorldSize() throws Exception {
        ...
        for(int target:new int[]{500,2000,5000}) {
            ...
            // As one command: the 100 blocks queued, then applied as the world does (every 1,024 events and at the tick).
            FluidRuntimeDiagnostics.reset();submitNanos=0;applyNanos=0;t=System.nanoTime();
            for(int i=0;i<20;i++){int m=lines++;int x=X0+6*(m%16),z=Z0+2*(m/16);String layout="RPPPR";
                for(int k=0;k<layout.length();k++){long id=ledger.nextIdentity();long a=System.nanoTime();registry.submit(List.of(new WorldTopologyLedger.Edit(id,record(id,x+k,z,layout.charAt(k)=='R'?Kind.RESERVOIR:Kind.PIPE,PhysicalFluidTopology.Direction.EAST))),id+1,null);submitNanos+=System.nanoTime()-a;
                    if(registry.unapplied()>=FluidWorldAuthority.APPLY_BATCH){a=System.nanoTime();registry.applyPending();applyNanos+=System.nanoTime()-a;}}}
            {long a=System.nanoTime();registry.applyPending();applyNanos+=System.nanoTime()-a;}
            double command=(System.nanoTime()-t)/1e6/100;var counters=FluidRuntimeDiagnostics.sample();
            say(String.format(Locale.ROOT,"N=%5d %-26s %8.3f ms per block (submit %.3f, apply %.3f) batches %d events %d compiled devices %d",n,"100 blocks as one command",command,submitNanos/1e6/100,applyNanos/1e6/100,counters.get("topologyBatches"),counters.get("topologyEvents"),counters.get("compiledDevices")));
