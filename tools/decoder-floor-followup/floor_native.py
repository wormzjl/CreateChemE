"""Isolated Java build and execution for the decoder-floor study.

The core is rebuilt from this worktree's sources with only Gson on the compiler classpath, exactly as
the predecessor campaign did, so no stale project bytecode can satisfy a dependency and the only
difference from the sealed core is the registered source delta.
"""
from floor_common import *
from floor_register import core_sources, verify_plan
import argparse
import os
import subprocess

JAVA = Path('C:/Program Files/Java/jdk-21.0.11/bin')
CORE_CLASSES = ROOT / 'build/neural-decoder-floor/native-core-v1'
