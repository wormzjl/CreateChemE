...
/** Offline full-anchor absolute-output Transformer with two or four width-64 blocks. Parser-owned weights are immutable after validation.
 * Every prediction owns all scratch arrays; cooperative cancellation is checked inside matrix work.
 */
final class V3CapacityInitializer implements V3NeuralInitializer {
    private static final String REVISION = "v3-capacity-1";
    ...
    private V3CapacityInitializer(Document model) { this.model = model; }

    static V3CapacityInitializer read(InputStream stream) throws IOException {
        ...
        if (m == null || !REVISION.equals(m.featureRevision) || !"capacity-transformer".equals(m.modelType)
        ...
        var result = new V3CapacityInitializer(m);
        ...
        for (int i = 0; i < m.layerCount; i++) {
            String p = "blocks." + i + ".";
            if (m.modelType.equals("capacity-transformer")) {
    ...
    Raw raw(V3ColumnInput input, V3SolveControl control) {
        ...
        for (int block = 0; block < model.layerCount; block++) {
            ...
            if (model.modelType.equals("capacity-transformer")) {
