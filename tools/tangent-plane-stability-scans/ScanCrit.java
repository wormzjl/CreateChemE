...
public class ScanCrit {
    public static void main(String[] args) {
        ...
        for (double T = 130; T <= 185; T += 5) for (double P = 2.5e6; P <= 6.0e6; P += 0.02e6) for (double zn = 0.01; zn < 0.995; zn += 0.02) {
            double[] z = {1 - zn, zn};
            ...
            if (r.verdict() == TangentPlaneStability.Verdict.UNRESOLVED) { unres++; System.out.printf("UNRESOLVED T=%.1f P=%.4f z=%.3f tm=%+.3e brute=%+.3e it=%d ev=%d dv=%d x=%.5f root=%s feed=%s%n", T, P/1e6, zn, r.minimumTangentPlaneDistance(), 0.0, r.iterations(), r.kernelEvaluations(), r.derivativeEvaluations(), r.trialComposition()[1], r.trialRoot(), r.feedRoot()); }
