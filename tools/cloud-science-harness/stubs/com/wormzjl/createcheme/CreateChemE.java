...
// Harness stand-in for the mod entry point (NeoForge @Mod class; its constructor and event handlers need Minecraft).
// Kept: the config fields and their static builder block, calculationLoggingEnabled, the FluidOptions record,
// fluidOptions() and the columnV3* accessors, copied verbatim from src/main/java/com/wormzjl/createcheme/CreateChemE.java
// (harness.sh checks every line between the VERBATIM markers against the real file before each compile), built on the
// ModConfigSpec stand-in, whose values read as their declared defaults. LOGGER is a stand-in that prints to stdout.
...
public final class CreateChemE {
    ...
    public static final HarnessLogger LOGGER = new HarnessLogger();
    private CreateChemE() {}

    /** Minimal slf4j-style logger: formats {} placeholders, prints the trailing Throwable's toString. */
    public static final class HarnessLogger {
        private static void log(String level, String format, Object... arguments) {
            var text = new StringBuilder("[harness-log ").append(level).append("] ");
            int argument = 0, at = 0;
            Throwable thrown = arguments.length > 0 && arguments[arguments.length - 1] instanceof Throwable t ? t : null;
            int usable = thrown == null ? arguments.length : arguments.length - 1;
            while (true) {
                int next = format.indexOf("{}", at);
                if (next < 0 || argument >= usable) { text.append(format, at, format.length()); break; }
                text.append(format, at, next).append(arguments[argument++]);
                at = next + 2;
            }
            if (thrown != null) text.append(" | ").append(thrown);
    ...
    // VERBATIM-BEGIN fields and static block
