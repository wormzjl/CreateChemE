 ...
 *
 * <p>This is the campaign's parity gate. The default variant must reproduce the archived F0 seeds of the
 * predecessor campaign exactly, which proves that the rebuilt core, the derived initializer and this
 * study's source delta leave the production decode untouched before any solver time is spent. The
 * serializer settings match the evaluation probe so the emitted seeds are directly comparable.</p>
 */
public final class V3BudgetDecodeCheck {
    private static final Gson JSON = new GsonBuilder().serializeNulls().create();

    private V3BudgetDecodeCheck() {}
