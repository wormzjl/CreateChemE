...
public final class ConservativeTransport {
    ...
    /** The phases (ids of {@link FluidThermodynamics}) a draw vector supplies, in phase-id order. */
    private static int[] drawnPhases(double[] rates) {
        int count=0;for(double rate:rates)if(rate>0)count++;
        int[] phases=new int[count];int at=0;for(int phase=0;phase<rates.length;phase++)if(rates[phase]>0)phases[at++]=phase;
        return phases;
    }
    /** The weights of a draw's phases in its stream, {@code r_phase/|q|}; exactly 1 for a draw of one phase. */
    private static double weight(double[] rates,int[] phases,int phase) {
        if(phases.length==1)return 1;
        double total=0;for(int p:phases)total+=rates[p];
        return rates[phase]/total;
    }
    /** The mass share of solids in what {@code pipe} draws out of {@code donor} at {@code flow}: the bulk share
     * {@code solids/mass} (the double it always was) where it draws the bulk, and otherwise its phases' solid shares
     * weighted by their mass flows (none in the gas; a liquid's share of the solids over its stream's mass). */
    private static double drawnSolidShare(FluidThermodynamics model,double[] rates,FluidThermodynamics.State donor) {
        if(rates==null)return donor.solidMoments().mass()/donor.mass();
        var phases=drawnPhases(rates);double share=0;
        for(int phase:phases)if(phase!=FluidThermodynamics.GAS)share+=weight(rates,phases,phase)*(FluidThermodynamics.phaseSolidShare(donor,phase)*donor.solidMoments().mass()/model.phaseMass(donor,phase));
        return share;
    }
    /**
    ...
    /**
     * The mass fractions of what a phase-port outflow draws (decision D11: the priority stream), frozen from the Newton's
     * converged candidate: its phases' fractions ({@link #phaseFractions}) weighted by their mass flows at the solved flow,
     * exactly one phase's where it draws one. Booked as a fixed donor on the pinned flow, so {@code dt q w} leaves the
     * donor and reaches the receiver and mass is conserved whatever the weights.
     */
    private static double[] streamFractions(FluidThermodynamics model,double[] rates,FluidThermodynamics.State donor,
                                            PassiveNetwork.Reservoir reservoir,List<SolidInventory.Key> populationKeys,double[] molecularWeight,int components) {
        var phases=drawnPhases(rates);
        if(phases.length==1)return phaseFractions(model,phases[0],donor,reservoir,populationKeys,molecularWeight,components);
        var result=new double[components+populationKeys.size()];
        for(int phase:phases){var w=phaseFractions(model,phase,donor,reservoir,populationKeys,molecularWeight,components);double weight=weight(rates,phases,phase);
    ...
    private static Projection reconstruct0(PassiveNetwork graph,List<FluidThermodynamics.State> candidate,double[] solvedFlows,double[] heads,
        ...
        for(int edge=0;edge<flows.length;edge++) {
            ...
            if(phase)drawn[edge]=streamFractions(model,rates,candidate.get(donor),graph.reservoirs().get(donor),populationKeys,molecularWeight,components);
            double deliveredRate=pipe.filter()==null?massRate:massRate*(1-drawnSolidShare(model,rates,candidate.get(donor)));
        ...
        for(int node=0;node<nodes;node++) {
            ...
            for(int c=0;c<components;c++){double w=solved[c][index[node]];if(w<0||!Double.isFinite(w)){System.out.println("NEG node="+node+" c="+c+" w="+w+" old="+old[node][c]+" cand="+PhaseLayout.totalAmounts(candidate.get(node))[c]+" flows="+java.util.Arrays.toString(solvedFlows)+" drawn="+java.util.Arrays.deepToString(draws));throw new SparseNewton.Nonconvergence("Negative transport reconstruction");}fractions[node][c]=w;total+=w;molesPerKg+=w/molecularWeight[c];}
        ...
        for(int edge=0;edge<flows.length;edge++) {
            ...
            else{specificEnthalpy=0;for(int phase:phases)specificEnthalpy+=weight(rates,phases,phase)*model.phaseSpecificEnthalpy(upstream,phase,null);}
            ...
            if(pipe.filter()!=null){
                ...
                if(drawn[edge]==null)capturedEnergy=moved*(upstream.solidMoments().enthalpy(upstream.temperature(),upstream.pressure())/upstream.mass()+upstream.solidMoments().mass()/upstream.mass()*PassiveStepSolver.GRAVITY*upstreamZ);
                    ...
                    for(int phase:phases)if(phase!=FluidThermodynamics.GAS){double share=FluidThermodynamics.phaseSolidShare(upstream,phase),streamMass=model.phaseMass(upstream,phase);
                        perMass+=weight(rates,phases,phase)*(share*upstream.solidMoments().enthalpy(upstream.temperature(),upstream.pressure())/streamMass+share*upstream.solidMoments().mass()/streamMass*PassiveStepSolver.GRAVITY*upstreamZ);}
            ...
            if(pipe.control() instanceof FlowControl.Pump pump&&flows[edge]>0){var suction=states.get(pipe.first());
                ...
                else{double specificVolume=0;for(int phase:phases)specificVolume+=weight(rates,phases,phase)*(FluidThermodynamics.phaseVolume(suction,phase)/model.phaseMass(suction,phase));
    ...
    private static double[] pinnedFlows(PassiveNetwork graph,List<FluidThermodynamics.State> candidate,double[] flows,FluidThermodynamics model,double[][] draws) {
        ...
        while(!queue.isEmpty()) {
            ...
            for(int edge=0;edge<flows.length;edge++) {
                ...
                if(receiver==node){double delivered=rate*factor[edge];if(pipe.filter()!=null)delivered*=1-drawnSolidShare(model,draws==null?null:draws[edge],candidate.get(donor));if(junction[donor])fromJunctions+=delivered;else fromOthers+=delivered;}
