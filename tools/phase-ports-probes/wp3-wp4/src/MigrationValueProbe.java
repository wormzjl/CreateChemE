...
public class MigrationValueProbe {
    ...
    static void flowControl() {
        var t=fcRun(101325,200000,NEW?new FlowControl.Compressor(.001,3,1):new FlowControl.Pump(.001,gasSetting(500000,101325),1),.1);
        ...
        var n=fcRun(300000,101325,NEW?new FlowControl.Compressor(.001,3,1):new FlowControl.Pump(.001,500000,1),.1);
        var s=fcRun(101325,800000,NEW?new FlowControl.Compressor(.001,3,1):new FlowControl.Pump(.001,100000,1),.1);
    ...
    static void networkRegime() {
        ...
        for(double p:new double[]{1.49e6,1.51e6}){var r=new PassiveStepSolver(nm).solve(new PassiveNetwork(List.of(source,gas(2,350,p,PassiveNetwork.NodeKind.VOID)),List.of(new PassiveNetwork.Pipe(1,0,1,g,NEW?new FlowControl.Compressor(.001,1.5,1):new FlowControl.Pump(.001,setting,1)))),1,NOOP);
        ...
        var recycle=new PassiveNetwork(List.of(gas(1,350,150000,PassiveNetwork.NodeKind.RESERVOIR),gas(2,350,150000,PassiveNetwork.NodeKind.RESERVOIR)),List.of(new PassiveNetwork.Pipe(1,0,1,g,NEW?new FlowControl.Compressor(.001,1.5,1):new FlowControl.Pump(.001,500000,1)),new PassiveNetwork.Pipe(2,1,0,g)));
        ...
        var sonic=new PassiveNetwork(List.of(gas(1,350,101325,PassiveNetwork.NodeKind.GENERATOR),gas(2,350,101325,PassiveNetwork.NodeKind.VOID)),List.of(new PassiveNetwork.Pipe(1,0,1,new PipeResistance.Geometry(.1,.01,0,0),NEW?new FlowControl.Compressor(1,6,1):new FlowControl.Pump(1,500000*nm.pumpReferenceDensity()/(ss.mass()/ss.volume()),1))));
    ...
    static void backwardEuler() {
        ...
        var graph=new PassiveNetwork(List.of(suction,d),List.of(new PassiveNetwork.Pipe(1,0,1,new PipeResistance.Geometry(10,.05,.000045,0),NEW?new FlowControl.Compressor(.001,3,.8):new FlowControl.Pump(.001,500000*nm.pumpReferenceDensity()/(suction.state().mass()/suction.state().volume()),.8))));
