 ...
 */
public final class TangentPlaneStability {
    ...
    public TangentPlaneStability(PengRobinsonKernel kernel) { this(kernel, Settings.DEFAULT); }

    public TangentPlaneStability(PengRobinsonKernel kernel, Settings settings) {
    ...
    private Result run(double temperatureKelvin, double pressurePascal, double vaporPressurePascal, boolean branched,
        ...
        for (int trial = 0; trial < trialCount; trial++) {
            Outcome outcome = runTrial(w, trial);
            trials++;
            Point end = w.current;
    ...
    private static boolean admissible(Workspace w, PengRobinsonKernel.Evaluation evaluation, PengRobinsonKernel.Root root) {
        ...
        return PhaseIdentification.vapourLike(PhaseIdentification.parameter(evaluation, w.temperature, w.vaporPressure));
    }
