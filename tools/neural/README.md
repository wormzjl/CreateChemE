...
The [hybrid mechanistic/Transformer investigation](hybrid-transformer-investigation.md) tests one native material-completion step after the frozen Transformer. On **20 fixed dry/no-side-draw TRAIN inputs**, with ten workers and unchanged request budgets, neural-first qualifies **19/20 versus 17/20**, gaining two cases with no losses. Reconstruction is admissible on only **8/20** inputs; the rest retain their raw seeds. Mean request time is **2.18 versus 2.68 seconds**, but unchanged-seed cases also show timing shifts, so this is not a controlled speedup estimate. The investigation compares deeper hybrid architectures and proposes full validation before any promotion. No model fitting or runtime-default change was made.
...
The comparison's [recommendation erratum v1](generation-comparison-recommendation-erratum-v1.md) corrects the proposed material-loss formulation: current V3 represents pumparounds through prescribed stage-heat duties. The sealed findings and measured results remain unchanged.
