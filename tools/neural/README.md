...
The [transformer investigation](transformer-investigation.md) expands fitting to 805 qualified columns, freezes a replacement 252-case holdout, characterizes coverage and label ambiguity, and compares a small transformer with a parameter-matched MLP on CUDA across three seeds. This is an offline pilot; native solver convergence has not yet been tested. The [protocol](transformer-protocol.md), [machine-readable summary](transformer-pilot-summary.json), and [cache manifest](transformer-cache-manifest.json) preserve the experiment. Use a project-local environment with `requirements-transformer.txt` for GPU training.
