...
/** Offline CPU transformer/MLP candidate. Parser-owned weights are immutable after validation.
 * Every prediction owns all scratch arrays; cooperative cancellation is checked inside matrix work.
 */
final class V3ColumnTransformerInitializer implements V3NeuralInitializer {
    private static final String REVISION = "v3-column-transformer-1";
    ...
    static V3ColumnTransformerInitializer read(InputStream stream, V3FactorizedNeuralFeatures.DecodeOptions decode) throws IOException {
        ...
        if (m == null || !REVISION.equals(m.featureRevision) || !("transformer".equals(m.modelType) || "mlp".equals(m.modelType))
        ...
        for (String name : List.of("xm", "xscale", "ym", "yscale", "gm", "gscale")) {
            int count = name.startsWith("x") ? 96 : name.startsWith("y") ? 85 : 74;
        ...
        for (int i = 0; i < 2; i++) {
            String p = "blocks." + i + ".";
            if (m.modelType.equals("transformer")) {
        ...
        long expected = m.modelType.equals("transformer") ? 83800 : 83288;
    ...
    Raw raw(V3ColumnInput input, V3SolveControl control) {
        ...
        for (int block = 0; block < 2; block++) {
            ...
            if (model.modelType.equals("transformer")) {
