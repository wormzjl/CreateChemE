...
/** Immutable gen3 CPU inference: phase totals, normalized compositions, and optional learned seed support. */
public final class V3FactorizedNeuralInitializer implements V3NeuralInitializer {
    ...
    private V3FactorizedNeuralInitializer(Document model) { this.model = model; }

    /** Parses a bounded model artifact. Arrays are parser-owned and never exposed. Caller owns the stream. */
    public static V3FactorizedNeuralInitializer read(InputStream stream) throws IOException {
        byte[] bytes = stream.readNBytes(MAX_BYTES + 1);
        if (bytes.length > MAX_BYTES) throw new IllegalArgumentException("Factorized neural model exceeds size budget");
        Document m;
        try { m = new Gson().fromJson(new String(bytes, StandardCharsets.UTF_8), Document.class); }
        catch (JsonParseException malformed) { throw new IllegalArgumentException("Malformed factorized neural model", malformed); }
        if (m == null || !V3FactorizedNeuralFeatures.REVISION.equals(m.featureRevision)
                ...
                || !Double.isFinite(m.presenceThreshold) || m.presenceThreshold < 0 || m.presenceThreshold > .5
                || m.traceFloorFraction != V3FactorizedNeuralFeatures.TRACE_FLOOR_FRACTION
                || m.traceFloorFraction != V3TruncationSupport.TRACE_FLOOR_FRACTION)
            throw new IllegalArgumentException("Invalid factorized neural manifest");
        boolean anyBranch = false;
        for (boolean seen : m.branchesSeen) anyBranch |= seen;
        if (!anyBranch) throw new IllegalArgumentException("Factorized neural model has no trained phase branch");
        int global = V3FactorizedNeuralFeatures.globalWidth(m.components.size());
        int node = V3FactorizedNeuralFeatures.nodeWidth(m.components.size());
        int output = V3FactorizedNeuralFeatures.outputWidth(m.components.size());
        ...
        if (weights > 2_000_000) throw new IllegalArgumentException("Factorized neural parameter budget exceeded");
        ...
        if (m.designConstraints != null) {
            ...
            if (!Double.isFinite(limits.minimumNodePressurePascal) || limits.minimumNodePressurePascal <= 0
                ...
                throw new IllegalArgumentException("Invalid factorized neural design constraints");
        }
        return new V3FactorizedNeuralInitializer(m);
    }
    ...
    @Override public Optional<V3NeuralSeed> predict(V3ColumnInput input, V3SolveControl control) {
        ...
        double[][] nodes = V3FactorizedNeuralFeatures.nodes(input, branch);
        ...
        control.checkpoint();
        try { return Optional.of(V3FactorizedNeuralFeatures.decode(input, model.propertyRevision, branch, output, model.presenceThreshold)); }
    ...
    private static long network(Layer[] layers, int width, int expectedOutput) {
        if (layers == null || layers.length < 1 || layers.length > 6) throw new IllegalArgumentException("Invalid factorized neural layers");
        long parameters = 0;
        for (Layer layer : layers) {
            if (layer == null || layer.weights == null || layer.weights.length < 1 || layer.weights.length > 1024
                ...
                throw new IllegalArgumentException("Invalid factorized neural layer");
            parameters += (long) width * layer.weights.length + layer.weights.length;
            if (parameters > 2_000_000) throw new IllegalArgumentException("Factorized neural parameter budget exceeded");
            for (double[] row : layer.weights) vector(row, width, false);
            width = layer.weights.length; vector(layer.bias, width, false);
        }
        if (width != expectedOutput) throw new IllegalArgumentException("Invalid factorized neural output dimension");
        return parameters;
    }
    ...
    private static void vector(double[] values, int width, boolean positive) {
        if (values == null || values.length != width || Arrays.stream(values).anyMatch(x -> !Double.isFinite(x) || positive && x <= 0))
            throw new IllegalArgumentException("Invalid factorized neural vector");
    }
    private static final class Document {
        ...
        double presenceThreshold, traceFloorFraction;
