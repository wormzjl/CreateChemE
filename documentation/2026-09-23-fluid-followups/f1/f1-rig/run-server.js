// F1 copy of the WP5 driver (logs under f1-logs/rig/, probe counts of the WP5 probes with six-tank fills). One WP5 dedicated-server run: fresh copy of the template world, runServer (no rendering in the JVM), scenario
...
const rig = __dirname, logs = path.resolve(rig, '..', 'f1-logs', 'rig', runId);
const run = path.join(worktree, 'run'), level = 'bench-' + runId, template = path.join(rig, 'template', 'bench-template');
const EXPECTED = {empty: 0, rest100: 500, through100: 700, fill100: 800, mixed100: 700, rest1000: 5000, pure100: 600, probe: 237, probe2: 528, probe3: 144, probe4: 264};
