#!/usr/bin/env bash
# F2 gates, one Gradle invocation at a time, logs in f2-logs/gate-<tag>-<name>.log. Usage: gates.sh <tag> [names...]
cd "D:/Minecraft/Modding/1.21/CreateChemE/.claude/worktrees/agent-ae139e4fc1b184b36" || exit 2
L=documentation/fluid-followups/f2-logs
TAG=${1:-dev}; shift
...
for n in $NAMES; do
  case $n in
    ...
    gametest) G gametest runFluidGameTestServer -PfluidGameTestRunId=f2-$TAG ;;
  esac
done
