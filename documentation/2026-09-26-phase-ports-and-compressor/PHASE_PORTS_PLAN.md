...
- Batch: `2026-09-26-phase-ports-and-compressor`. Status: **In progress** since 2026-09-26 (planned 2026-09-26; WP0-WP6, D9-D13 done on branch `claude/phase-ports-compressor`, awaiting the owner's in-game check and merge, see section 7 and `PHASE_PORTS_REVIEW.md` "WP6: gates and close").
...
### 3.4 What a port draws: phases by priority (decision D11)

*(Rewritten 2026-09-26 under owner decision D11, which supersedes the "outlet phase absent" closure this section proposed and WP2 implemented; implementation and measurements in `PHASE_PORTS_REVIEW.md` D11, defaults A20-A32 in `DECISION_LOG.md`. The WP2 text is kept in the review's WP2 section and in git history.)*

A port never closes for want of a phase. Every non-BULK port has a **priority order** of the vessel's phases (read on the step's start state):
- a **bottom (LIQUID) port**: the heavier of the two liquids (hydrocarbon liquid, free water; by their own densities), then the lighter, then the gas;
- a **top (VAPOR) port**: the gas, then the lighter liquid, then the heavier;
- a **side (BULK) end**: the bulk, as before.

Each liquid carries its volume share of the solids (A20). The bottom-port order is a decant (A2 withdrawn).

**Capacity.** A phase can be drawn through a port over one step only up to what the vessel held of it at the step start: `c_phi = m_phi,start / (N dt)`, with `N` the vessel's phase-port ends whose connection may carry outflow by the static rule (equal share, zero reserve; A21) and `dt` the solve's step; infinite in a rate solve.

**Priority stream.** For an outflow `q`, the first phase of the order supplies `min(q, c_1)`, the second `min(max(q - c_1, 0), c_2)`, and so on; the last phase the vessel holds takes whatever the earlier ones do not (A22). A phase the vessel does not hold has no capacity and is skipped; if it holds none of the three, the port draws the bulk (A31). The stream's composition, solids and enthalpy are the mass-flow-weighted mixture of the drawn phases, its density the no-slip mixture's and its viscosity the volume-flow average (A24). A phase drawn at its capacity (pinned) is drawn at the start state, the marginal one at the end state (A23). So a slowly fed drain draws liquid at its feed rate with gas making up the rest (steady breakthrough, no alternation), and a vent on a liquid-full tank overflows the lighter liquid.

**Where in the pass loop.** Which phases are pinned and which is marginal (the *segment*) is frozen per active-set pass, decided from the pass's start flows and seeds, and re-decided by the pass loop from the converged flows, like the junction donors and the head columns (A25): each Newton residual is smooth. The segment is part of the cycle key, not of the workspace key. A pass starts inside its mixture's velocity cap (A26); a failed pass whose vessel's pinned segments draw a phase's whole content is reseeded without it (A27).

**What goes.** The availability mask, the 1 %/0.5 % band, the port refusal in `boundaryAllowed` and its reopen allowance, the throttle as a landing device and the start clamp to it, A9's absent stream. `INLET_EMPTY` (3.6) goes: a pump fed from a dry bottom port sees gas, and D1's rule refuses it. **Liquid-full vessels** no longer pressurise behind a top port: a tank fed liquid overflows through its vent (the withdrawn paragraph of this section).

**Velocity cap and a device downstream.** A mixture is capped at a volume flow `A v_min`, `v_min` the smallest velocity limit of its phases (A24); a single-phase draw keeps its own stream's cap (a vapour draw from a wet tank is limited by the vapour's acoustic bound). A pump or compressor fed by a port: 3.6 (WP3 decides its wrong-phase rule on the leading phase or the draw).

**Static readers** (columns, sign tests, start flows, the start-of-solve closures, the reopen test, the junction seeds, the solid mobility check) read the port's leading phase, the priority stream at a vanishing flow (A28).
...
- *Reason.* The view shows the reason next slice: "ERROR: pump inlet not liquid (vapour 34 % by volume, from <source device>)", computed read-only from the committed graph with the same walk (a pure function of graph and states, shared with the solver). *Implemented as `InletPhase.reason`: "ERROR: pump inlet not liquid (vapour 34.0 % by volume, from node <id>)", the id of the node the walk ended at; WP5 maps it to a device name and passes the committed interval's duration (the supply of a phase port is a draw over the slice, A37).* *(Amended 2026-09-26 after WP3/WP4, `PHASE_PORTS_REVIEW.md` WP3 + WP4.)*
...
- *Implemented in WP5 (`PhysicalFluidTopology.port`, A46): only tank ends carry ports; the filter's positive-side remap keeps them (it rebuilt pipes with a constructor that resets ports); two touching tanks never connect, so a "stack" is tank - pipe - tank. (Amended 2026-09-26 after WP5, `PHASE_PORTS_REVIEW.md` WP5.)*
...
- *WP5 also needed: the block item, the device block entity's valid blocks, the creative tab, the pickaxe and Create `non_movable` tags; the model is the pump's with vanilla textures overridden (no mod texture exists, A52). (Amended 2026-09-26 after WP5.)*
...
- **Tank.** A line per connected face on the tank page: "Top: gas outlet (open | closed: no vapour)", "Side: mixed outlet", "Bottom: liquid outlet (open | closed: no liquid)", with the last-interval flow of that connection. The rows are derived read-only from the committed graph: pipes whose first or second end is the tank and their ports, availability recomputed from the committed state by the same stateless rule, flows from the last result, as `view` already sums them (`FluidWorldAuthority.java:286-288`). *Under D11 no port closes: implemented in WP5 as "Top: gas outlet" / "drawing gas | drawing liquid (overflow)", "Side: mixed outlet" / "drawing all phases together", "Bottom: liquid outlet (head X kPa)" / "drawing water | drawing oil | drawing gas", "receiving" for net inflow, with the flow; the phase drawn is the one that carried the most mass in the connection's last-interval pipe transfer (A47). (Amended 2026-09-26 after WP5.)*
- **Wording.** "Bulk withdrawal" wording where it exists becomes "Side outlets draw all phases together". *No player-facing string had it; the phrase is in the tank page's outlet hint. (Amended 2026-09-26 after WP5.)*
...
- `IslandCertificate` digests ports and the compressor (`:341-354`). *Verified in WP5: both since WP1/WP4, header `-graph-identity-2`, no change.*
- *Wire protocol fluid-6 -> fluid-7 (the ratio control and the tank's outlets, A50); JSON `"compressor"` reads `limit` as the ratio (A51). (Amended 2026-09-26 after WP5.)*
...
|---|---|---|---|---|
| ~~port available / port closed (phase share against `phi_open`)~~ | *superseded by D11: no port closes; there is no availability decision* | | | |
| port order and capacities (D11) | none (a draw rule) | step start state | the step | A phase is drawn up to what the start state held of it (`c = m_start/(N dt)`); a phase appearing mid-step is drawn from the next step (DELAYED, nothing bottled: the port draws the next phase meanwhile) |
| port segment (which phases are pinned, which is marginal; D11) | per pass, like the junction donors | the pass's start flows and seeds | the pass; re-decided from the converged flows | Part of the cycle key; a pass starts inside its mixture's cap; measured settling in `PHASE_PORTS_REVIEW.md` D11 |
| a one-way run shut by `closeDeadHeads` | reopening | step end state | the step, via the basis plan's `boundaryReopen` retry | The BROKEN class 3/3' of 8.8 (c). *D11: no port refusal remains, so the reopen test reads the static rule only* |
| phase drawn to zero within the step | the pinned phase leaves at its start content | the pass | the step | *D11:* the end state lacks the phase; a pass whose seed keeps it and fails is reseeded without it (A27); the next step's order skips it |
...
| ~~`INLET_EMPTY`~~ | *removed by D11: a dry bottom port feeds the pump gas, which D1's `INLET_WRONG_PHASE` refuses* | | | |
...
- *D11 (2026-09-26): a phase port drew its phases by priority, each at most what the vessel held of it at the step start (pinned phases at the start state, the marginal one at the end state); no port closes.* *(Superseded:)* - A phase port drew only its phase, at the end-state composition of that phase, and never below its reserve by more than the flash moved the interface within the step. A closed port carried zero outflow for the whole step; inflow may be nonzero.
- A pump or compressor in `INLET_WRONG_PHASE` ~~or `INLET_EMPTY`~~ carried exactly zero for the whole ~~step or~~ slice (measured, fixtures 4, 5, 7). *(Amended 2026-09-26 after WP3/WP4, `PHASE_PORTS_REVIEW.md` WP3 + WP4.)*
...
*(Fixtures 2 and 3 superseded by D11 (2026-09-26): `PhasePortPriorityTest` replaces `PhasePortClosureTest`; `PHASE_PORTS_REVIEW.md` D11 section 7.)*
...
*(D11: no throttle and no `INLET_EMPTY`; a drained bottom port feeds the pump gas, refused by D1.)* 5. *Liquid pump drains a tank.* A pump from a LIQUID port runs on target, then on HEAD_LIMIT as the throttle binds, then `INLET_EMPTY`; no hold. *Measured under D11 and A37 (`InletPhaseTest`): TARGET, TARGET, HEAD_LIMIT (the port's water runs out within the third slice, admitted with 1 % of its draw gas), then `INLET_WRONG_PHASE` from the next slice; no hold.* *(Amended 2026-09-26 after WP3/WP4, `PHASE_PORTS_REVIEW.md` WP3 + WP4.)*
...
8. *Replay:* a certified island with a refused pump ~~and a closed port~~ *(D11: no port closes; the fixture puts the refusal in its hysteresis band instead, so the replay needs the committed mode)* *(Amended 2026-09-26 after WP3/WP4, `PHASE_PORTS_REVIEW.md` WP3 + WP4.)* reproduces the every-interval run bitwise (the `CausalModuleCoordinatorTest` harness shape, 8.9 (a)).
...
2. a pump on a tank's side (refused, reason shown), moved to the bottom face (runs, then shows "no liquid at the suction" when drained); *under D11 the drained bottom port feeds the pump gas and the reason reads "ERROR: pump inlet not liquid (vapour ... % by volume, from reservoir at x, y, z)"; the three scenarios as the owner's check list: `PHASE_PORTS_REVIEW.md` WP5, 11. (Amended 2026-09-26 after WP5.)*
...
| WP2 | *Superseded by D11 (commit `548a9bf`).* Phase-absent state (3.4): availability in `boundaryAllowed`, throttle, key, `boundaryReopen` link allowance. **WP2 done 2026-09-26** on branch `claude/phase-ports-compressor`, not merged: commit `d836cf2`: `PhasePortClosureTest` 15/15 (fixtures 2 and 3 at 5 s and 0.1 s, the throttle, the vanishing phase), fluid suites 428/428 with the junction lines bitwise, exact regression 0.000e+00, 38/38 adjacent, GameTest and mcpCompat sources compile; the band lands just below `phi_open` (section 3.4 amendment, owner options in the review). | Fixtures 2 and 3; no nonconvergence in them at 5 s and 0.1 s; fluid suites unchanged | 1.5 |
| D9 | Level head at the bottom port (owner decision D9, option B of `LEVEL_HEAD_REVIEW.md`): `portHead` = `g m_c H/V` at a LIQUID port of a vessel, H = 1 m fixed, through the WP1 helper at all eleven driving-pressure sites; no unknown, row or structural entry. **D9 done 2026-09-26** on branch `claude/phase-ports-compressor`, not merged: commit `fdf3574` (`WIP phase-ports D9: level head at the bottom port (option B, H = 1 m)`); `LevelHeadTest` 8/8 (drain on the head alone, dead-headed against the head, reopen by the head, two-tank equalisation, manometer, pump into a bottom port, bitwise replay), fluid suites 448/448 with the junction lines bitwise, exact regression 0.000e+00, 38/38 adjacent, GameTest and mcpCompat sources compile; four `PhasePortClosureTest` assertions re-baselined to the bottom-port pressure (A19); the physical-topology re-baselines wait for WP5. | Fixtures 1-5 of the review's section 5; gas-only islands bitwise; fluid suites green | 1-1.25 |
| D11 | Ports carry mixed phases by priority (owner decision D11, section 3.4 rewritten): priority order per port, per-step capacities `m_start/(N dt)`, priority stream with a per-pass segment, pinned phases at the start state; WP2's mask, band, refusal, reopen allowance and throttle removed. **D11 done 2026-09-26** on branch `claude/phase-ports-compressor`, not merged: commit `548a9bf` (`WIP phase-ports D11: priority streams at ports (replaces the absent-phase closure)`): `PhasePortPriorityTest` 10/10 (drain to gas, fed drain, liquid-full overflow, pentane over water both ports, gas-only bottom port, closed lines bitwise, capacity split), `PhasePortClosureTest` deleted, one `LevelHeadTest` fixture restated (A32), fluid suites 443/443 with the junction lines bitwise, exact regression 0.000e+00, 38/38 adjacent, GameTest and mcpCompat sources compile; a WP1 unbacked-trace defect fixed (A30). | Fixtures (1)-(7) of the D11 brief; all-BULK bitwise; fluid suites green | 1.5 |
| D12 | One-way generators from the cold start (owner decision D12; options O2 then O1 of `EXTREME_TOPOLOGY_TESTS.md` section 6): the cold rate seed closes before its pass 0 the generator runs a one-way pressure estimate shows receiving and starts its junctions from that estimate (`PassiveStepSolver.closeColdReceivingGenerators`, `oneWayPressures`); the first step's `closeIllegalStarts` then starts every step with one-way generators; O1 not needed. **D12 done 2026-09-26** on branch `claude/cold-start-generators-wip` (worktree off `7051391`), not merged: commit `418ca7e` (`WIP phase-ports D12: one-way generators from the cold start`); the three full extreme-topology fixtures integrate at the default cap (40 x 5 s, 400 x 0.1 s, every oracle; `FULL_CASES_OPEN_DEFECT = false`), the 4.4 scan 120/120 runs (base 49/120 first intervals); harness all green with the 33 junction lines identical and chain-100 0.000e+00, the WP1/D9 bitwise probe byte-identical; Gradle gates not run in the worktree; **merged into `claude/phase-ports-compressor` 2026-09-26 (`aaa6624`), all gates passed after the D13 merge** (review "D11+D12+D13 merge"). | The three full fixtures at 100 m/s, 40 x 5 s and 400 x 0.1 s, every oracle; junction lines bitwise; chain-100 exact | 1 |
| D13 | Vent gate defect (owner decision D13, option A of `PHASE_PORTS_REVIEW.md` "Vent gate defect"): a point the reconstruction's equation gate refuses is re-solved once from the converged point on a fresh Jacobian at 1e-2 x the tolerance and gated again before the step is refused (`PassiveStepSolver.polishRefusedPoint`); `SolverDiagnostics` counts gate rejections and polishes. **D13 done 2026-09-26** on branch `claude/vent-gate-polish-wip` (worktree `scratchpad/wt-vent`), not merged: commit `4a71629`; `VentGatePolishTest` 3/3, the D9 manometer vents B; cloud harness 216 + 237 + 38 + chain-100 0.000e+00 with the junction lines identical, D9 bitwise probe byte-identical. **Merged into `claude/phase-ports-compressor` 2026-09-26 (`334ca78`)**, its post-checks restated to the D11 pass loop's; fluid suites 446/446 with the junction lines bitwise, exact regression 0.000e+00, 38/38 adjacent, GameTest and mcpCompat sources compile; no equation-gate refusal left in any suite (review "D11+D12+D13 merge"). | Zero equation-gate rejections on the vented manometer at 5 s and 0.1 s; every step the gate accepted before is bitwise unchanged; Gradle gates after the merge | 0.5 |
| WP3 | Liquid-only pump (3.6): supply walk, `INLET_WRONG_PHASE` ~~/ `INLET_EMPTY`~~ (D11), suction-stream densities everywhere, target-above-supply start, reason text; migrate the WP0 gas-pump tests. **WP3 done 2026-09-26 together with WP4** on branch `claude/phase-ports-compressor`, not merged: commit `fc9574d` (`WIP phase-ports WP3+WP4: liquid-only pump, compressor, inlet-phase refusal`): `InletPhase` (walk, measures, decision, reason; a port supplies its D11 draw of the target over the slice, A37), the mode decided once per slice by the interval solver on the committed state and endpoint mode and held as a closed device; `InletPhaseTest` 6/6 (walk, thresholds, fixtures 4, 5, 8); the 15 C tests migrated, the R test a refusal, the three B re-baselines made (crude bubble point 117.8 kPa at 298.15 K, 226.0 kPa at 350 K by flash); fluid suites 459/459 with the junction lines bitwise and MIXED_GAS_COST 286, exact regression 0.000e+00, 39/39 adjacent (the 38 plus one new), GameTest and mcpCompat sources compile. | Fixtures 4, 5, 8; the classified pump suites green or re-baselined as agreed | 1.5 |
| WP4 | Compressor (3.7): `Mover`, `Compressor`, ratio limit, work, inlet check, topology kind, invalid-cycle rule. **WP4 done 2026-09-26 with WP3** (same commit `fc9574d`): sealed `FlowControl.Mover` read by every pump site of the solver, `Compressor` (limit `(r_max - 1) P_s`, isothermal work as heat in the Newton and the reconstruction, 1 %/0.2 % condensed and solid refusal), `Kind.COMPRESSOR` in the topology, device validity, certificate digest case, the codec refusing to write one until format 6; `CompressorTest` 5/5 (fixtures 6, 7; the heating hand check measured +93.47 K against 93.6 K) and two topology tests. | Fixtures 6, 7; migrated gas-transfer tests green; energy ledger | 1.5 |
| WP5 | Runtime (section 4): face-to-port compile, block and assets, codec format 6, controls and wire, certificate digest, GUI for pump, compressor and tank. **Re-baselines due with the face-to-port compile (D9, `PHASE_PORTS_REVIEW.md` D9 section 4):** once DOWN faces compile to LIQUID ports the level head reaches the physical-topology fixtures fed through a tank's bottom, and four assertions move: `ElevatedBlockLineIslandTest` :216, :220, :229 (rising, rising filter, rising pump lines) and `DeadHeadedLineIslandTest` :243 (C.2's :234), each re-stated to `P_tank + g m_w H/V = P_generator - rho g LIFT` (headspace about 7 kPa lower by `LEVEL_HEAD_REVIEW.md`'s estimate, 0.72 m of water; the code-built equivalent measured -5.61 kPa for a tank charged at 150 kPa); plus the level-head review's fixture 6 (face-to-port and head on a compiled stack) and the optional tank-page head line. **WP5 done 2026-09-26** on branch `claude/phase-ports-compressor`, not merged: commit `9744a45` (`WIP phase-ports WP5: faces to ports, compressor block, format 6, GUI`): face-to-port compile (A46), `fluid_compressor` block, format 6 and wire fluid-7, mover reasons and limits, tank page lines with the head (A47-A49); the four re-baselines made (headspace 6.95 kPa lower, 8.61 kPa pumped; bottoms at the old expectations); fixture 6 as a tank over a void (a two-tank stack passes its gas up the pipe); fluid suites 472/472 with the junction lines bitwise and MIXED_GAS_COST 286, exact regression 0.000e+00, 45/45 adjacent, **fluid GameTests 33/33 run in the container** (the three new included), `build -x test` green; in-game MCP check left to the owner (`PHASE_PORTS_REVIEW.md` WP5, 11). | `PhysicalFluidTopologyTest` face cases, codec round trip (ports, compressor, new modes), fluid runtime suite, the three new GameTests | 2 |
| WP6 | Gates and close: fluid suites, exact regression, GameTests, MCP in-game scenarios, tooling cleanup, review `PHASE_PORTS_REVIEW.md`, `CHANGELOG.md` `[Unreleased]` line, INDEX rows. **WP6 done 2026-09-26** on branch `claude/phase-ports-compressor`, not merged: commit `cb41860` (`WIP phase-ports WP6: cleanup and final gates`): the extreme-topology open-defect reproduction and the unread WP1 condensed-stream builders detached to `tools/phase-ports-probes/` with reattach patches, stale source text corrected; gates after the cleanup (fluid suites 472/472 with the junction lines bitwise and MIXED_GAS_COST 286, exact regression 0.000e+00, 45/45 adjacent, fluid GameTests 33/33 on a fresh world, the full `test` task 1165/1166 with the one failure pre-existing on the base, `build -x test` green, harness all green); the MCP in-game scenarios are the owner's (no display in the container), review "WP6: gates and close" (f) lists what the merge needs. | All gates recorded in the review | 1 |
...
*(R6 withdrawn by D11 (2026-09-26): a liquid-full vessel overflows through its vent.)* - **R6 Liquid-full vessels with only top ports** rise in pressure on liquid inflow (by design, 3.4). A player may read it as a bug; the tank screen names it.
...
| `.fallingLine` :84-90 | :201, :268 | UP, VAPOR | water in from above | U (inflow; the tank end's column only enters the reverse, forbidden, direction) *(WP5 measured: the rest moves +4.4 mPa, and `DeadHeadedLineIslandTest`'s falling line -0.11 mPa, from the VAPOR end's leading-phase column (A28) at the landing resolution; assertions unchanged, A54. Amended 2026-09-26 after WP5.)* | U (no head at VAPOR) |
