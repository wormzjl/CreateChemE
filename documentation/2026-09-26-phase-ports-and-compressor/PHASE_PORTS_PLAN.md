...
*(D11: no throttle and no `INLET_EMPTY`; a drained bottom port feeds the pump gas, refused by D1.)* 5. *Liquid pump drains a tank.* A pump from a LIQUID port runs on target, then on HEAD_LIMIT as the throttle binds, then `INLET_EMPTY`; no hold.
