...
Milestone update (2026-09-06): steps 1 and 2 have been implemented and checked in the isolated V3 branch. The [milestone report](V3_LITERATURE_CDU_MILESTONE_1.md) supersedes the historical source-access and attachment-stage uncertainties below. Source connections are verified; the registered 19-component package agrees with native fugacity and thermodynamically consistent PR78 enthalpy. Installed DWSIM default enthalpy has a separately measured inconsistency, so the next column reference must explicitly restore a consistent caloric treatment on replay. No column solve is claimed.

Reference-harness update (2026-09-06): native caloric registration, NS/SR heat-sign checks, VLL condenser checks and checked small-case replay are implemented. The [reference milestone report](V3_LITERATURE_CDU_REFERENCE_MILESTONE.md) records successful manufactured scaffolds and rejected full-tower attempts. The full native CDU remains unqualified; this gate stays open before full-target comparison.
