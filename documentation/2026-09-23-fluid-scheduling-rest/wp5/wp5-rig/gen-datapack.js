...
function grid1000(kind) {
  const cmds = [];
  for (let n = 0; n < 1000; n++) cmds.push(...line(kind, X0 + 6 * (n % 16), Z0 + 2 * Math.floor(n / 16)));
  return cmds;
}
const scenarios = {
  empty: [],
  ...
  probe: (() => { const c = []; let row = 0; for (const [k, m] of [['REST', 1], ['PURE', 2], ['THROUGH', 4], ['FILL', 8], ['FLAT', 16]]) for (let i = 0; i < m; i++) c.push(...line(k, X0, Z0 + 2 * row++)); return c; })(),
};
// Probe 2: fill variants. 1 F1, 2 F2, 4 F3, 8 FILL (six tanks), 16 gravity lines with one tank, 32 with three.
scenarios.probe2 = (() => { const c = []; let row = 0; for (const [k, m] of [['F1', 1], ['F2', 2], ['F3', 4], ['FILL', 8]]) for (let i = 0; i < m; i++) c.push(...line(k, X0, Z0 + 2 * row++));
