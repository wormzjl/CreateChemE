...
class TransientQualificationTest {
    ...
    @Test void freeWaterDisappearanceMatchesRefinedTrajectoriesAcrossThreeIntervals() throws Exception {
        double[] n=new double[22];n[20]=40;n[21]=5;var unit=model.flashTP(298.15,150000,n,()->{});for(int i=0;i<n.length;i++)n[i]/=unit.volume();
        ...
        for(int interval=1;interval<=3;interval++) {
            ...
            assertEquals(n[21],candidate.reservoirs().get(1).inventory().moles()[21],1e-8);
        }
    ...
    @Test void liquidCompressionWetCrudeReversalAndPumpMatchRefinedTrajectories() throws Exception {
        double[] water=new double[22];water[21]=1;
        var wet=Arrays.copyOf(V3PengRobinsonThermo.fromRegisteredPackage("createcheme:tjl20_methane").crudeFeed("createcheme:tia_juana_light_methane").moleFractions(),22);wet[20]=.1;wet[21]=.2;
        double[] gas=new double[22];gas[20]=1;
        double[] smallHeadspace=new double[22];smallHeadspace[20]=.05;smallHeadspace[21]=54000;
