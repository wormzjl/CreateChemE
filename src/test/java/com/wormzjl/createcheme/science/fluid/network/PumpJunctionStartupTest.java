...
class PumpJunctionStartupTest {
    @Test void waterAndAmbientCrudeStartThroughAZeroStoragePumpInletWithoutInventingUpstreamNitrogen() {
        ...
        for(String id:List.of("water","createcheme:tia_juana_light_methane")) {
            ...
            var unit=model.flashTP(298.15,101325,composition,()->{});for(int c=0;c<composition.length;c++)composition[c]/=unit.volume();
            var source=new PassiveNetwork.Reservoir(1,-60,model.flashTP(298.15,101325,composition,()->{}),PassiveNetwork.NodeKind.GENERATOR);
            var junction=new PassiveNetwork.Reservoir(2,-60,source.state(),PassiveNetwork.NodeKind.JUNCTION);
            var receiver=new PassiveNetwork.Reservoir(3,-60,model.initialNitrogenCharge(1,298.15,101325,()->{}));
