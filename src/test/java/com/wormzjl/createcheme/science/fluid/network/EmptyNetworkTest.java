...
class EmptyNetworkTest {
    ...
    private PassiveNetwork.Reservoir empty() {
        ...
        return new PassiveNetwork.Reservoir(1,0,guess,PassiveNetwork.NodeKind.RESERVOIR,new PassiveNetwork.Inventory(1,new double[22],0));
    }
    @Test void isolatedEvacuatedReservoirAdvancesWithoutEosEvaluationOrInventedGas() {
        ...
        assertArrayEquals(new double[22],result.graph().reservoirs().getFirst().inventory().moles());assertEquals(0,result.graph().reservoirs().getFirst().inventory().internalEnergy());
    ...
    @Test void unsupportedVacuumFillingIsExplicitAndCannotPublishOutflowFromTheRetainedGuess() {
        ...
        assertArrayEquals(new double[22],empty.inventory().moles());assertEquals(0,empty.inventory().internalEnergy());
    }
}
