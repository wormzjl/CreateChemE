...
class NetworkRegimeTest {
    ...
    private PassiveNetwork.Reservoir gas(long id,double t,double p,int component,PassiveNetwork.NodeKind kind) {
        var n=new double[22];n[component]=1;var unit=model.flashTP(t,p,n,()->{});n[component]/=unit.volume();
        return new PassiveNetwork.Reservoir(id,0,model.flashTP(t,p,n,()->{}),kind);
    }
    @Test void twoDifferentFeedsMixAtAZeroHoldupJunction() {
        var graph=new PassiveNetwork(List.of(gas(1,350,220000,0,PassiveNetwork.NodeKind.GENERATOR),gas(2,400,210000,20,PassiveNetwork.NodeKind.GENERATOR),
                gas(3,350,195000,20,PassiveNetwork.NodeKind.JUNCTION),gas(4,350,180000,20,PassiveNetwork.NodeKind.VOID)),List.of(
    ...
    @Test void reversingOneFeedSwitchesJunctionUpwindingAndSerialValvesRemainSolvable() {
        var reversed=new PassiveNetwork(List.of(gas(1,350,220000,0,PassiveNetwork.NodeKind.GENERATOR),gas(2,400,170000,20,PassiveNetwork.NodeKind.RESERVOIR),
                gas(3,350,195000,20,PassiveNetwork.NodeKind.JUNCTION),gas(4,350,180000,20,PassiveNetwork.NodeKind.VOID)),List.of(
                new PassiveNetwork.Pipe(1,0,2,geometry),new PassiveNetwork.Pipe(2,1,2,geometry),new PassiveNetwork.Pipe(3,2,3,geometry)));
        var back=new TrBdf2StepSolver(model).solve(reversed,.1,()->{});assertTrue(back.massFlows()[1]<0);
        assertEquals(0,PhaseLayout.totalAmounts(back.states().get(2))[20],1e-9);assertTrue(back.inventories().get(1).moles()[0]>0);
        var serial=new PassiveNetwork(List.of(gas(1,350,250000,20,PassiveNetwork.NodeKind.GENERATOR),gas(2,350,200000,20,PassiveNetwork.NodeKind.JUNCTION),gas(3,350,101325,20,PassiveNetwork.NodeKind.VOID)),List.of(
    ...
    @Test void aRejectedTrialRestartsFromTheSameConservedStateAtTheSmallerStep() {
        var graph=new PassiveNetwork(List.of(gas(1,350,200000,20,PassiveNetwork.NodeKind.RESERVOIR),gas(2,350,101325,20,PassiveNetwork.NodeKind.RESERVOIR)),List.of(new PassiveNetwork.Pipe(1,0,1,geometry)));
    ...
    @Test void pumpShutoffUsesAddedPressureAndTankRecycleIsAllowed() {
        var source=gas(1,350,1e6,20,PassiveNetwork.NodeKind.GENERATOR);
        for(double p:new double[]{1.49e6,1.51e6}) {
            var graph=new PassiveNetwork(List.of(source,gas(2,350,p,20,PassiveNetwork.NodeKind.VOID)),List.of(new PassiveNetwork.Pipe(1,0,1,geometry,new FlowControl.Pump(.001,500000,1))));
        ...
        var recycle=new PassiveNetwork(List.of(gas(1,350,150000,20,PassiveNetwork.NodeKind.RESERVOIR),gas(2,350,150000,20,PassiveNetwork.NodeKind.RESERVOIR)),List.of(
    ...
    @Test void bypassCanPreventAValveFromMaintainingItsUpstreamSetpoint() {
        var graph=new PassiveNetwork(List.of(gas(1,350,180000,20,PassiveNetwork.NodeKind.RESERVOIR),gas(2,350,101325,20,PassiveNetwork.NodeKind.RESERVOIR)),List.of(
    ...
    @Test void hotNitrogenCanEvaporateTheLastFreeWaterWithoutDiscardingIt() {
        double[] n=new double[22];n[20]=40;n[21]=5;var initial=model.flashTP(298.15,150000,n,()->{});double scale=1/initial.volume();for(int i=0;i<n.length;i++)n[i]*=scale;
        var tank=new PassiveNetwork.Reservoir(2,0,model.flashTP(298.15,150000,n,()->{}));assertTrue(tank.state().waterLiquid()>0);
        var graph=new PassiveNetwork(List.of(gas(1,500,500000,20,PassiveNetwork.NodeKind.GENERATOR),tank),List.of(new PassiveNetwork.Pipe(1,0,1,geometry)));
        for(int i=0;i<3;i++)graph=new PassiveIntervalSolver(model).solve(graph,20,PassiveIntervalSolver.Settings.defaults(),()->{}).graph();
        var end=graph.reservoirs().get(1);assertEquals(0,end.state().waterLiquid(),1e-9,"T="+end.state().temperature()+" P="+end.state().pressure());assertEquals(n[21],end.inventory().moles()[21],1e-8);
    }
    @Test void hydrocarbonLiquidCanDisappearIntoTheSharedGasPhase() {
        ...
        double[] n=new double[22];n[20]=30;n[pentane]=50;var unit=model.flashTP(298.15,150000,n,()->{});for(int i=0;i<n.length;i++)n[i]/=unit.volume();
        var tank=new PassiveNetwork.Reservoir(2,0,model.flashTP(298.15,150000,n,()->{}));assertTrue(tank.state().liquidVolume()>0);
        var graph=new PassiveNetwork(List.of(gas(1,500,500000,20,PassiveNetwork.NodeKind.GENERATOR),tank,gas(3,350,101325,20,PassiveNetwork.NodeKind.VOID)),
    ...
    @Test void cancellationCannotLeavePartialTransfersAndExcessVelocityIsConservativelyLimited() {
        ...
        var sonic=new PassiveNetwork(List.of(gas(1,350,101325,20,PassiveNetwork.NodeKind.GENERATOR),gas(2,350,101325,20,PassiveNetwork.NodeKind.VOID)),List.of(
                new PassiveNetwork.Pipe(1,0,1,new PipeResistance.Geometry(.1,.01,0,0),new FlowControl.Pump(1,500000,1))));
