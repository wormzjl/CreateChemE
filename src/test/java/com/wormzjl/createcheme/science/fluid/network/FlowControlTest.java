...
class FlowControlTest {
    ...
    private PassiveStepSolver.Result run(double pa,double pb,int component,FlowControl control,double dt) {
        ...
        for(double p:new double[]{pa,pb}){double[] n=new double[21];n[component]=1;var unit=model.flashTP(350,p,n,()->{});n[component]/=unit.volume();nodes.add(new PassiveNetwork.Reservoir(nodes.size(),0,model.flashTP(350,p,n,()->{})));}
    ...
    @Test void pressureValveClosesBelowTargetAndRegulatesAboveIt() {
        var closed=run(101325,80000,20,new FlowControl.PressureValve(175000),.1);
        assertEquals(FlowControl.Mode.CLOSED,closed.modes().getFirst());assertEquals(0,closed.massFlows()[0],1e-10);
        var regulated=run(200000,101325,20,new FlowControl.PressureValve(175000),.1);
