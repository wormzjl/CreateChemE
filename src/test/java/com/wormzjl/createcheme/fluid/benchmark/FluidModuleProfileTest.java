...
class FluidModuleProfileTest {
    @Test void profileActualDeferredReceiver() throws Exception {
        ...
        var saved=FluidSavedData.load(tag,resolve).checkpoint();var module=saved.modules().getFirst();var binding=saved.moduleBindings().stream().filter(b->b.buffer().equals(module.definition().firstProduct())).findFirst().orElseThrow();
