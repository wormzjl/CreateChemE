...
class WorldTopologyLedgerTest {
    ...
    @Test void independentLaterEventCanCommitWithoutAdvancingTheBlockedEventsTimestamp() {
        ...
        var applied=ledger.applyReady(second.event().id(),List.of(),List.of(),new double[22],3);ledger.commit(applied);
        ...
        assertEquals(Set.of(1L,2L),restored.latest().keySet());restored.commit(restored.applyFirst(List.of(),List.of(),new double[22],3));assertEquals(Set.of(1L,2L),restored.active().keySet());
    ...
    @Test void sharedIslandDependenciesAndRemovalReplacementPositionsCannotBeReordered() {
        ...
        assertEquals(List.of(remove.event()),ledger.readyEvents());assertThrows(IllegalStateException.class,()->ledger.applyReady(replacement.event().id(),List.of(),List.of(),new double[22],4));
        ledger.commit(ledger.applyFirst(List.of(),List.of(),new double[22],4));assertEquals(List.of(replacement.event()),ledger.readyEvents());ledger.commit(ledger.applyFirst(List.of(),List.of(),new double[22],4));
        ...
        assertEquals(List.of(a.event()),ledger.readyEvents());assertThrows(IllegalStateException.class,()->ledger.applyReady(b.event().id(),List.of(),List.of(),new double[22],4));
    }
    ...
    @Test void constructionAndDestructionCountOnceAndNeverReuseTheOldIdentity() {
        ...
        double[] w=new double[22];for(int c=0;c<22;c++)w[c]=c==21?model.waterMolecularWeight:model.hydrocarbon.molecularWeight(c);
