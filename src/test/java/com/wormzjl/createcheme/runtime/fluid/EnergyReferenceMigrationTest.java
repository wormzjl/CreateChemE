...
class EnergyReferenceMigrationTest {
    ...
    private double[] offsets(){var values=new double[com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK+1];values[com.wormzjl.createcheme.science.material.MaterialTestBasis.NITROGEN]=1000;values[com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK]=-500;return values;}
    ...
    @Test void explicitMigrationPreservesPhysicalStatesSignedBoundaryEnergyPendingRemaindersAndClocks() {
        ...
        double[] n=new double[com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK+1],weights=new double[com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK+1];for(int c=0;c<com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK+1;c++)weights[c]=c==com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK?model.waterMolecularWeight:model.hydrocarbon.molecularWeight(c);n[com.wormzjl.createcheme.science.material.MaterialTestBasis.NITROGEN]=.02/weights[com.wormzjl.createcheme.science.material.MaterialTestBasis.NITROGEN];n[com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK]=.08/weights[com.wormzjl.createcheme.science.material.MaterialTestBasis.NETWORK];
        ...
        var checkpoint=new FluidCheckpointCodec.Checkpoint(List.of(new FluidCheckpointCodec.IslandEntry("minecraft:overworld","createcheme:tjl20_methane_nitrogen",1e-9,island)),new BufferedTransfers.Snapshot(3,Map.of(receiver,buffer),Map.of(transfer,pending)));
        ...
        assertTrue(oldBoundaries.stream().anyMatch(b->b.moles()[com.wormzjl.createcheme.science.material.MaterialTestBasis.NITROGEN]<0));for(int i=0;i<oldBoundaries.size();i++)assertEquals(oldBoundaries.get(i).totalEnergyJoule(),newBoundaries.get(i).totalEnergyJoule(),1e-8);
