...
class FluidSaveCompatibilityTest {
    @Test void oldCoreOnlySaveStillLoadsAndResavesWithoutLosingInventoryOrInventingTopology() {
        ...
        var legacy=new FluidSavedData(checkpoint,key->model).save(new CompoundTag(),null);assertFalse(legacy.contains("Topology"));assertEquals(1,legacy.getInt("FluidFormat"));
