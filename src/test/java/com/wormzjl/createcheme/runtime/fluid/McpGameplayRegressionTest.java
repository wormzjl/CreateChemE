...
class McpGameplayRegressionTest {
    @Test void clampedDrainContinuesAfterTheTankBecomesAlmostLiquidFull() throws Exception {
        ...
        assertEquals(1,result.advancedSeconds());assertTrue(result.graph().reservoirs().getFirst().inventory().moles()[20]>0);
        ...
        for(int i=0;i<graph.reservoirs().size();i++) {
            ...
            assertArrayEquals(expected.inventory().moles(),actual.inventory().moles(),1e-8*Math.max(1,expected.inventory().moles()[21]));
        }
        ...
        assertTrue(continuing.reservoirs().getFirst().inventory().moles()[20]>0);
    }
    @Test void savedSonicRefusalAdvancesWithVelocityClampingAndKeepsCanonicalBalances() throws Exception {
        ...
        double[] before=new double[22],after=new double[22],external=new double[22];double beforeEnergy=0,afterEnergy=0,externalEnergy=0;
        for(var boundary:result.boundaries()){var n=boundary.moles();for(int c=0;c<22;c++)external[c]+=n[c];externalEnergy+=boundary.totalEnergyJoule();}
        for(int i=0;i<graph.reservoirs().size();i++)if(graph.reservoirs().get(i).kind()==PassiveNetwork.NodeKind.RESERVOIR) {
            ...
            double ma=0,mb=0;for(int c=0;c<22;c++){before[c]+=na[c];after[c]+=nb[c];double mw=c==21?model.waterMolecularWeight:model.hydrocarbon.molecularWeight(c);ma+=na[c]*mw;mb+=nb[c]*mw;}
        ...
        for(int c=0;c<22;c++)assertEquals(before[c]+external[c],after[c],1e-10+1e-8*Math.max(before[c],after[c]));
    ...
    @Test void belowSeaLevelNitrogenTankAcceptsWaterAfterAnIdlePeriodAndPressureEdit() {
        ...
        assertEquals(nitrogen.vapor()[20],result.graph().reservoirs().getFirst().inventory().moles()[20],1e-8);
