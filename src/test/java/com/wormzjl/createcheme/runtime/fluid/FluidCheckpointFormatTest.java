...
/**
 * Checkpoint format 3 (plan section 3.5 and requirement R5; section 5 item 7, replacing the legacy-save gate):
 * clocks, material, fences, pending events, modules and certificates round-trip exactly; every certificate field is
 * validated; certificates are discarded, keeping inventory, when certificates are off or their signature changed;
 * a certified island's payload is copied rather than encoded while it stays certified; a thousand certified islands
 * load without a solve burst; and a save taken during a property hold restarts awake from the held state. Older
 * formats are refused with the instruction to create a fresh world; nothing reads them.
 */
class FluidCheckpointFormatTest {
    ...
    private static int index(CompoundTag tag,long island) {
        var list=(ListTag)tag.get("Islands");
        for(int i=0;i<list.size();i++)if(list.getCompound(i).getLong("Id")==island)return i;
        throw new AssertionError("no island "+island);
    }
    private static CompoundTag islandTag(CompoundTag tag,long island){return ((ListTag)tag.get("Islands")).getCompound(index(tag,island));}
    ...
    @Test void clocksMaterialFencesPendingEventsModulesAndCertificatesRoundTripExactly() {
        ...
        var tag=new FluidSavedData(checkpoint,world,key->model).save(new CompoundTag(),null);
        assertEquals(3,tag.getInt("FluidFormat"));assertEquals(1_234,tag.getLong("Epoch"));
        assertEquals(FluidCheckpointCodec.AWAKE,islandTag(tag,4).getLong("Base"));
        assertEquals(checkpoint.islands().get(1).snapshot().certificate().orElseThrow().baseTick(),islandTag(tag,2).getLong("Base"));
        ...
        assertEquals(tag,loaded.save(new CompoundTag(),null),"a loaded checkpoint saves back to the same bytes");
    }
    ...
    @Test void aRestoredCertificateContinuesWithoutASolveExactlyAsIfNeverSaved() {
        ...
        var tag=FluidCheckpointCodec.encode(original.checkpoint(),1_234,key->model);
        // The payload holds the certificate base; the load materialises it to the saved committed tick.
        var base=original.stored(2).certificate().orElseThrow().saved().orElseThrow().interval().result().graph();
        var payload=FluidCheckpointCodec.payloadJson(tag,index(tag,2));
        for(var node:payload.getAsJsonObject("graph").getAsJsonArray("nodes")) {
            long id=node.getAsJsonObject().get("id").getAsLong();var inventory=node.getAsJsonObject().getAsJsonObject("inventory");
            var expected=base.reservoirs().stream().filter(n->n.id()==id).findFirst().orElseThrow().inventory();
            assertEquals(expected.internalEnergy(),inventory.get("internalEnergy").getAsDouble(),0,"the payload records the base of node "+id);
        }
        ...
        var decoded=FluidCheckpointCodec.decode(tag,key->model);
    ...
    /** Plan 3.5: every new field is validated on load, and an invalid one refuses the load with a clear reason. */
    @Test void everyInvalidCertificateFieldIsRefused() {
        ...
        var tag=FluidCheckpointCodec.encode(rig.checkpoint(),1_234,key->model);
        var island=islandTag(tag,2);long base=island.getLong("Base"),committed=island.getLong("Committed");
        var certificate=island.getCompound("Certificate");long horizon=certificate.getLong("Horizon"),start=certificate.getLong("Start");
        assertTrue(base<committed&&committed<horizon);
        record Case(String expected,Consumer<CompoundTag> edit) {}
        var cases=List.<Case>of(
                new Case("later than its committed tick",t->islandTag(t,2).putLong("Committed",base-1)),
                new Case("earlier than its committed tick",t->islandTag(t,2).getCompound("Certificate").putLong("Horizon",committed-1)),
                new Case("invalid certificate base",t->islandTag(t,2).putLong("Base",-2)),
                new Case("carries a certificate",t->islandTag(t,2).putLong("Base",FluidCheckpointCodec.AWAKE)),
                new Case("lacks its certificate record",t->islandTag(t,2).remove("Certificate")),
                new Case("is empty or negative",t->islandTag(t,2).getCompound("Certificate").putLong("Start",base)),
                new Case("disagrees with its result",t->islandTag(t,2).getCompound("Certificate").putLong("Start",start-20)),
                new Case("since <= base",t->islandTag(t,2).getCompound("Certificate").putLong("Since",base+1)),
                new Case("since <= base",t->islandTag(t,2).getCompound("Certificate").putLong("Since",-1)),
                new Case("unknown certificate kind",t->islandTag(t,2).getCompound("Certificate").putString("Kind","DORMANT")),
                new Case("field Horizon",t->islandTag(t,2).getCompound("Certificate").remove("Horizon")),
                new Case("Invalid certificate signature",t->islandTag(t,2).getCompound("Certificate").putString("Graph","not a digest")),
                new Case("ahead of the checkpoint's world epoch",t->t.putLong("Epoch",1_233)));
        for(var c:cases) {
            var edited=tag.copy();c.edit().accept(edited);
            ...
            try{FluidCheckpointCodec.reseal(edited);}catch(IllegalArgumentException missing){assertTrue(missing.getMessage().contains(c.expected()),missing.getMessage());}
        ...
        for(var node:payload.getAsJsonObject("graph").getAsJsonArray("nodes"))if(node.getAsJsonObject().get("kind").getAsString().equals("RESERVOIR")){node.getAsJsonObject().getAsJsonObject("inventory").addProperty("internalEnergy",1.7e308);break;}
        for(var node:payload.getAsJsonObject("certifiedFrom").getAsJsonArray("nodes"))if(node.getAsJsonObject().get("kind").getAsString().equals("RESERVOIR")){node.getAsJsonObject().getAsJsonObject("inventory").addProperty("internalEnergy",-1.7e308);break;}
        assertTrue(assertThrows(IllegalArgumentException.class,()->FluidCheckpointCodec.decode(FluidCheckpointCodec.withPayload(tag,0,payload),key->model)).getMessage().contains("nonfinite energy delta"));
        // A fence before the committed tick.
        var fenced=FluidCheckpointCodec.payloadJson(tag,0);fenced.getAsJsonObject("fences").addProperty(UUID.randomUUID().toString(),committed-1);
        assertTrue(assertThrows(IllegalArgumentException.class,()->FluidCheckpointCodec.decode(FluidCheckpointCodec.withPayload(tag,0,fenced),key->model)).getMessage().contains("precedes the committed tick"));
    }
    ...
    @Test void aSignatureMismatchDiscardsTheCertificateAndKeepsTheInventory() {
        ...
        record Case(String reason,FluidThermodynamics model,CertificatePolicy policy,CompoundTag tag) {}
        // A graph-identity change: every run of pipe one metre longer, in the base and in the interval's start alike.
        var payload=FluidCheckpointCodec.payloadJson(tag,0);
        for(var name:List.of("graph","certifiedFrom"))for(var pipe:payload.getAsJsonObject(name).getAsJsonArray("pipes"))
            for(var section:pipe.getAsJsonObject().getAsJsonArray("sections"))section.getAsJsonObject().addProperty("length",section.getAsJsonObject().get("length").getAsDouble()+1);
        var cases=List.of(
        ...
        for(var c:cases) {
            var decoded=FluidCheckpointCodec.decode(c.tag(),key->c.model());
            countFromHere();
    ...
    /**
     * Plan 3.5: a certified island's payload is copied, not encoded, on every save while it stays certified; a solve,
     * a wake at the horizon and a new certificate make the next save encode it. (Tests run with the scheduler's
     * self-verification on, so every copied payload is also compared with a fresh encoding of its island.)
     */
    @Test void aCertifiedPayloadIsCopiedNotReencodedUntilASolveOrACertificate() {
        assertTrue(Boolean.getBoolean("createcheme.fluid.scheduler.verify"),"the copied payloads are verified against a fresh encoding");
        ...
        assertEquals(2,fourth.payloadsEncoded(),"the revalidated pair and the awake island are encoded");assertEquals(1,fourth.payloadsReused(),"the resting line is copied");
    ...
    @Test void aThousandCertifiedIslandsLoadWithoutASolveBurst() {
        ...
        long started=System.nanoTime();var tag=FluidCheckpointCodec.encode(checkpoint,300,key->model);long encoded=System.nanoTime();
        var decoded=FluidCheckpointCodec.decode(tag,key->model);long decodedAt=System.nanoTime();
        countFromHere();
        ...
        System.out.printf(Locale.ROOT,"1000 certified islands: encode %.1f ms, decode %.1f ms, register %.1f ms, %d bytes%n",(encoded-started)/1e6,(decodedAt-encoded)/1e6,(registered-decodedAt)/1e6,
                ((ListTag)tag.get("Islands")).stream().mapToLong(t->((CompoundTag)t).getByteArray("Payload").length).sum());
        FluidRuntimeDiagnostics.reset();
        loaded.run(1_099);
    ...
    @Test void holdSaveAndRestartKeepsCommittedFrozenAndDebtAndRequalifiesOverTheConfirmCount() {
        for(int confirm:new int[]{2,3}) {
            ...
            var tag=FluidCheckpointCodec.encode(rig.checkpoint(),rig.epoch[0],key->model);
            assertEquals(FluidCheckpointCodec.AWAKE,islandTag(tag,1).getLong("Base"));
            var decoded=FluidCheckpointCodec.decode(tag,key->model).islands().getFirst().snapshot();
    ...
    /** Format 2 and older are refused with the instruction to create a fresh world, and nothing in the tag is changed. */
    @Test void formatTwoAndOlderAreRefusedWithAFreshWorldInstructionAndLeftUntouched() {
        for(int format:new int[]{1,2}) {
            var old=new CompoundTag();old.putInt("FluidFormat",format);
            byte[] json=("{\"version\":"+format+",\"islands\":[],\"transfers\":{\"revision\":0,\"buffers\":[],\"pending\":[]}}").getBytes(StandardCharsets.UTF_8);
            old.putByteArray("Checkpoint",json);old.putByteArray("SHA256",new byte[32]);var before=old.copy();
            var refusal=assertThrows(IllegalArgumentException.class,()->FluidSavedData.load(old,key->model));
        ...
        var current=new FluidSavedData(new FluidCheckpointCodec.Checkpoint(List.of(),new BufferedTransfers.Snapshot(0,Map.of(),Map.of())),world(40),key->{throw new AssertionError("no model needed");}).save(new CompoundTag(),null);
