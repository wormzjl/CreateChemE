...
class FluidPacketCodecTest {
    ...
    @Test void nonfiniteOutOfDomainAndMalformedCompositionControlsCannotReachAnApply() {
        ...
        var accepted=new FluidNetwork.Controls(298.15,101325,.05,.000045,.01,500000,n);n[20]=0;assertEquals(1,accepted.composition()[20]);
    }
    @Test void aFullBranchHistoryFitsTheBoundedClientWireAndSurvivesTheActualJsonSchema() {
        double[][] amounts=new double[3][22];for(int p=0;p<3;p++)for(int c=0;c<22;c++)amounts[p][c]=(p+1)*(c+1)*1e-6;
        ...
        var names=new ArrayList<String>();for(int i=0;i<22;i++)names.add("component_"+i);
