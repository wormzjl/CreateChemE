...
public final class FluidHarnessGameTests {
    ...
    public static void gameplayPlacementTransfersFluidAndRemovalClosesThePersistentOwnershipLedger(GameTestHelper helper) {
        ...
        helper.succeedWhen(()->{
            if(phase[0]==0) {
                ...
                var restored=com.wormzjl.createcheme.runtime.fluid.FluidSavedData.load(tag,key->com.wormzjl.createcheme.science.fluid.thermo.FluidThermodynamics.forNetwork(com.wormzjl.createcheme.science.material.MaterialRuntime.active(),key.packageId(),key.compressibility()),data.store().reopen());
    ...
    public static void savedDataKeepsADepletedNitrogenChargeAndAtomicWritesKeepThePreviousCheckpoint(GameTestHelper helper) throws java.io.IOException {
        ...
        var data=new com.wormzjl.createcheme.runtime.fluid.FluidSavedData(checkpoint,key->model,store);data.setDirty();
        var path=folder.resolve(com.wormzjl.createcheme.runtime.fluid.FluidCheckpointStore.CORE_NAME);data.save(path.toFile(),helper.getLevel().registryAccess());
        var pack=folder.resolve(com.wormzjl.createcheme.runtime.fluid.FluidCheckpointStore.UNIT_DIRECTORY).resolve("fluid-1.pack");
        helper.succeedWhen(()->{
            helper.assertTrue(java.nio.file.Files.exists(path)&&java.nio.file.Files.exists(pack),"Waiting for the SavedData IO worker");
            try {
                store.flush();
                var loaded=com.wormzjl.createcheme.runtime.fluid.FluidSavedData.read(store.reopen(),key->model).orElseThrow().checkpoint().islands().getFirst().snapshot();
                ...
                try{com.wormzjl.createcheme.runtime.fluid.FluidCheckpointStore.atomicWrite(path,channel->{channel.write(java.nio.ByteBuffer.wrap(new byte[]{1,2,3}));throw new java.io.IOException("Injected checkpoint write failure");});}
                ...
                var packBytes=java.nio.file.Files.readAllBytes(pack);var corrupt=packBytes.clone();corrupt[corrupt.length-1]^=1;java.nio.file.Files.write(pack,corrupt);
                boolean rejected=false;try{com.wormzjl.createcheme.runtime.fluid.FluidSavedData.read(store.reopen(),key->model);}catch(IllegalArgumentException expected){rejected=expected.getMessage().contains("checksum");}
                helper.assertTrue(rejected,"A corrupt unit was accepted");
                java.nio.file.Files.write(pack,java.util.Arrays.copyOf(packBytes,packBytes.length-7));
                rejected=false;try{com.wormzjl.createcheme.runtime.fluid.FluidSavedData.read(store.reopen(),key->model);}catch(IllegalArgumentException expected){rejected=expected.getMessage().contains("half-written or damaged pack");}
                helper.assertTrue(rejected,"A truncated pack was accepted");
                java.nio.file.Files.delete(pack);java.nio.file.Files.delete(pack.getParent());java.nio.file.Files.delete(path);java.nio.file.Files.delete(folder);
