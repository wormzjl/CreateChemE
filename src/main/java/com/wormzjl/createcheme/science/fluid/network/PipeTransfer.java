...
public record PipeTransfer(long pipeId, Stream forward, Stream reverse) {
    ...
    public record Stream(double massKg, double[][] phaseMoles, double[] phaseVolumes) {
        public Stream {
    ...
    static List<PipeTransfer> sample(PassiveNetwork graph,List<FluidThermodynamics.State> states,double[] rates,double duration) {
        ...
        for(int i=0;i<rates.length;i++) {
            ...
            var moved=new Stream(mass,n,new double[]{fraction*state.liquidVolume(),fraction*state.waterVolume(),fraction*state.vaporVolume()});
    ...
    private static final class Mutable {
        ...
        private void add(Stream value,int direction,double weight) {
            mass[direction]+=weight*value.massKg;
            for(int p=0;p<3;p++) {
        ...
        private Stream stream(int direction){return new Stream(mass[direction],n[direction],volumes[direction]);}
    }
}
