...
public final class FluidCheckpointCodec {
     ...
     */
    static byte[] islandUnit(IslandEntry entry,long generation,Strings strings){return islandUnit(entry,generation,strings,null);}
    /** As {@link #islandUnit(IslandEntry,long,Strings)}, recording in {@code sections} (when not null) the bytes each
     * part of the unit took: a diagnostic for the payload breakdown of the F3 review. */
    static byte[] islandUnit(IslandEntry entry,long generation,Strings strings,Map<String,Integer> sections) {
        ...
        var w=new FluidUnitIO.Writer();var mark=new int[]{0};
        java.util.function.Consumer<String> section=name->{if(sections!=null)sections.merge(name,w.size()-mark[0],Integer::sum);mark[0]=w.size();};
        header(w,ISLAND,s.id(),s.revision(),generation);section.accept("header (identity, revision, generation)");
        w.varint(strings.ref(entry.dimension));w.varint(strings.ref(entry.packageId));w.f64(entry.compressibility);section.accept("dimension and package");
        w.text(s.status());section.accept("status");
        var a=s.allowance();w.varint(a.acceptedIntervals());w.varlong(a.advancedTicks());w.varint(a.capturedCadenceTicks());section.accept("allowance");
        ...
        section.accept("fences");
        int topologyStart=w.size();topology(w,graph);section.accept("graph topology (nodes and pipes, once)");int stateStart=w.size();state(w,graph);section.accept("graph state (inventories, phases, pipe masks and filters)");
        ...
        section.accept("anchor");
        if(result.isEmpty())w.u8(0);
        else {
            var r=result.orElseThrow();w.u8(1);
            w.f64(r.advancedSeconds());w.sparse(r.averageMassFlows());w.varint(r.acceptedSubsteps());w.varint(r.rejectedSubsteps());w.f64(r.pumpWorkJoule());section.accept("history: scalars and flows");
            ...
            section.accept("history: boundaries");
            ...
            modes(w,r.endpointModes());w.sparse(r.endpointHeads());w.u8(r.acceptance().ordinal());section.accept("history: reasons, modes, heads");
        ...
        section.accept("history: pipe transfers");
        if(persisted.isEmpty())w.u8(0);
            ...
            section.accept("certificate signature");
            related(w,saved.interval().before(),graph,main);
        }
        section.accept("certificate: interval start graph");
