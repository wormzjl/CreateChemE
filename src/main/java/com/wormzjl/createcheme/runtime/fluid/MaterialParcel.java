...
public final class MaterialParcel {
    private final SolidInventory solids;
    public SolidInventory solids(){return solids;}
    ...
    public MaterialParcel(double[] moles,double[] molecularWeights,double internalEnergy,EnergyReference reference) {
        this(moles,molecularWeights,internalEnergy,reference,SolidInventory.EMPTY);
    }
    public MaterialParcel(double[] moles,double[] molecularWeights,double internalEnergy,EnergyReference reference,SolidInventory solids) {
        this.solids=Objects.requireNonNull(solids);
        ...
        if(!Double.isFinite(total)||!Double.isFinite(massKg())||(total>0&&massKg()==0)||(total==0&&solids.empty()&&internalEnergy!=0))throw new IllegalArgumentException("Invalid empty/overflowed parcel");
    }
    ...
    public double massKg(){double total=solids.massKg();for(int i=0;i<moles.length;i++)total+=moles[i]*molecularWeights[i];return total;}
    public boolean empty(){return massKg()==0;}
    public MaterialParcel rebase(EnergyReference target){return new MaterialParcel(moles,molecularWeights,reference.rebase(internalEnergy,moles,target),target,solids);}
    ...
    public MaterialParcel plus(MaterialParcel other) {
        ...
        return new MaterialParcel(combined,molecularWeights,internalEnergy+other.internalEnergy,reference,solids.plus(other.solids));
    }
    ...
    public Portion takeMass(double maximumKg) {
        ...
        var solidPortion=solids.takeFraction(fraction);
        return new Portion(new MaterialParcel(delivered,molecularWeights,energy,reference,solidPortion.delivered()),new MaterialParcel(remaining,molecularWeights,internalEnergy-energy,reference,solidPortion.remaining()));
    }
    ...
    public List<MaterialParcel> split(double[] firstOutletFractions) {
        if(!solids.empty())throw new IllegalArgumentException("Solid-bearing parcels require an explicit separation contract");
