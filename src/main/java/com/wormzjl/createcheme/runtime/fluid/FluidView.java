...
public record FluidView(long identity,long inputRevision,long committedTick,long onlineTick,String status,State state,
                        ...
                        boolean hydraulicOwner,double intervalSeconds,String intervalQuality,List<PipeRoute> pipeRoutes) {
    ...
    public record State(double pressure,double temperature,double volume,double mass,double[] phaseVolumes,double[][] phaseMoles) {
        public State {phaseVolumes=phaseVolumes.clone();phaseMoles=Arrays.stream(phaseMoles).map(double[]::clone).toArray(double[][]::new);}
        ...
        static State from(FluidThermodynamics.State s) {
            ...
            return new State(s.pressure(),s.temperature(),s.volume(),s.mass(),new double[]{s.liquidVolume(),s.waterVolume(),s.vaporVolume()},n);
        }
    }
